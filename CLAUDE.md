...
**Decided 2026-07-30** (owner: product, ahead of M8's member-health slice — `system-design.md` §11.3, `FR-MEM-5/6`):

- **11. Member health signal v1 — single-signal calibration.** Principle 10's "shipped only after calibration" gate is satisfied for a first, narrow slice: `MemberHealthSignal.band` is computed from `daysSinceLastSpeech` alone — the last *approved* `SpeechSlot` where the member was speaker, on a meeting that has occurred. Reference date is `lastSpeechAt`, or `joinedAt` if they have never spoken (a grace period for new members). Thresholds: ≤60 days `healthy` · 61–90 `watch` · 91–180 `at_risk` · >180 `disengaged`. `MemberHealthSignal.dataSource` is stamped `"speech_only_v1"` so nobody later mistakes this for the full calibrated signal. The remaining inputs in `system-design.md` §11.3's interface (`attendanceRate90d`, `meetingsAttended90d`, `rolesFulfilled90d`, `daysSincePathwayProgress`, `duesStatus`) and all of `RetentionAlert` (the assign/action-log nudge workflow, `FR-MEM-6`) stay deferred — same reasoning as the already-deferred officer-activeness score: don't invent weights for signals with no real usage data yet. Visibility stays VPM-only for this slice (`membership.health_signal`, restricted); extending it to President and the member's mentor per `FR-MEM-5` is future work, not a blocker for VPM-only v1.
