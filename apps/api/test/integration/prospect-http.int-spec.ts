...
/** M4 Slice 1: the prospect pipeline. */
describe('M4 Slice 1: prospect pipeline (integration)', () => {
  ...
  it('a club_vpm creates a prospect — pipelineStatus defaults to new, deleteAfter is ~180 days out', async () => {
    const token = await jwtFor(vpmId);
    const res = await request(app.getHttpServer())
      .post(`/v1/clubs/${clubId}/prospects`)
  ...
  it('a club_vpm updates pipelineStatus to interested; setting it to joined directly is rejected', async () => {
    ...
    await request(app.getHttpServer())
      .patch(`/v1/clubs/${clubId}/prospects/${created.body.id}`)
      .set('Authorization', `Bearer ${token}`)
