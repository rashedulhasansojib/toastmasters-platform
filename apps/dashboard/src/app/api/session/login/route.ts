...
import { loginRequestSchema } from '@toastmasters/contracts';
import { callApi, extractSessionCookie, SESSION_COOKIE_OPTIONS } from '@/lib/session-proxy';

/** The dashboard's half of the BFF proxy (Slice 7) — see the Slice 7 plan's cross-origin-cookie note. */
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const upstream = await callApi('/v1/auth/login', {
    method: 'POST',
  ...
  if (!session) {
    return NextResponse.json(
      { message: 'Login succeeded but no session was issued' },
      {
        status: 502,
      },
    );
