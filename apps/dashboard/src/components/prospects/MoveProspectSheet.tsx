 ...
 */
export function MoveProspectSheet({
  prospect,
  open,
  onOpenChange,
  onMove,
  ...
  async function select(target: ProspectPipelineStatus) {
    if (!prospect) return;
    const ok = await onMove(prospect.id, target);
    ...
    >
      <div className="flex flex-col gap-1">
        {MOVABLE_STATUSES.map((status) => {
          const current = prospect.pipelineStatus === status;
          return (
            <button
              key={status}
              type="button"
