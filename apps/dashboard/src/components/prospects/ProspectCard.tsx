...
export function ProspectCard({
  ...
  const terminal = prospect.pipelineStatus === 'joined';
  const actionable = !redacted && (prospect.phone || wa || prospect.email);
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 p-3">
        ...
        >
          <Avatar name={prospect.fullName} photoUrl={prospect.photoUrl} size="md" />
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 truncate text-xs text-muted-foreground">
              {redacted ? (
                ...
                (prospect.phone ?? prospect.email ?? prospect.leadSource ?? 'No contact details')
              )}
            </p>
          </div>
        </Link>
        ...
        {terminal || redacted ? (
          <ProspectStatusBadge status={prospect.pipelineStatus} className="mt-1 shrink-0" />
        ) : (
          <button
            type="button"
